...
function collectKnownModels(config: any): Set<string> {
	...
	const providers = isPlainObject(config?.models?.providers)
		? config.models.providers
		: {};
	for (const [providerId, provider] of Object.entries(providers)) {
		const providerModels = Array.isArray((provider as any)?.models)
	...
	if (typeof defaultsModel === "string") {
		...
		const fallbacks = Array.isArray(defaultsModel.fallbacks)
			? defaultsModel.fallbacks
			: [];
		for (const fallback of fallbacks) addModelRef(models, fallback);
	}

	const defaultsModels = isPlainObject(config?.agents?.defaults?.models)
...
async function waitForPatchedModel(
	agentId: string,
	model: string,
): Promise<void> {
	...
	throw new Error(
		lastError || "Timed out waiting for Gateway to apply the new model",
	);
}
	...
	try {
		...
		if (snapshot?.valid === false || !isPlainObject(snapshot?.config)) {
			return NextResponse.json(
				{ ok: false, error: "Gateway config is invalid or unavailable" },
				{ status: 400 },
			);
		}
		...
		if (!baseHash) {
			return NextResponse.json(
				{ ok: false, error: "Missing baseHash from config snapshot" },
				{ status: 500 },
			);
		}
