...
const YUANBAO_PLUGIN_DIST_DIR = path.join(
	OPENCLAW_HOME,
	"extensions/openclaw-plugin-yuanbao/dist/src",
);
const DEFAULT_YUANBAO_API_DOMAIN = "bot.yuanbao.tencent.com";
const DEFAULT_YUANBAO_WS_URL =
...
async function _probeGatewayWebUi(
	port: number,
	token: string,
	timeoutMs = 5000,
	...
	try {
		...
		return {
			ok: false,
			error: err?.message || "Failed to reach gateway web UI",
		};
	} finally {
...
function resolveYuanbaoTestAccount(
	...
	const accounts =
		channelConfig?.accounts && typeof channelConfig.accounts === "object"
			...
			: {};
	const availableAccountIds = Object.keys(accounts).filter(
	...
	const defaultAccountId =
		...
		channelConfig.defaultAccount.trim()
			? channelConfig.defaultAccount.trim()
			: availableAccountIds.includes("default")
				? "default"
				: (availableAccountIds[0] ?? "default");
	...
	const identifier =
		typeof config.identifier === "string" ? config.identifier.trim() : "";
	const token = typeof config.token === "string" ? config.token.trim() : "";
	const apiDomain =
		typeof config.apiDomain === "string" && config.apiDomain.trim()
	...
	return {
		...
		account: {
			...
			mediaMaxMb:
				typeof config.mediaMaxMb === "number" && config.mediaMaxMb >= 1
					? config.mediaMaxMb
					: 20,
			historyLimit:
				typeof config.historyLimit === "number" && config.historyLimit >= 0
					? config.historyLimit
					: 100,
			config,
		},
	...
	if (!yuanbaoRuntimePromise) {
		yuanbaoRuntimePromise = Promise.all([
			importExternalModule(
				pathToFileURL(
					path.join(YUANBAO_PLUGIN_DIST_DIR, "yuanbao-server/http/request.js"),
				).href,
			),
			importExternalModule(
				pathToFileURL(
					path.join(YUANBAO_PLUGIN_DIST_DIR, "yuanbao-server/ws/client.js"),
				).href,
			),
			importExternalModule(
				pathToFileURL(
					path.join(YUANBAO_PLUGIN_DIST_DIR, "message-handler/outbound.js"),
				).href,
			),
...
async function testYuanbao(
	...
	const startTime = Date.now();
	const { accountId, account } = resolveYuanbaoTestAccount(
		channelConfig,
		preferredAccountId,
	);
	...
	if (!account.appKey || !account.appSecret) {
		return {
			...
			ok: false,
			error:
				"Yuanbao credentials missing. Configure channels.yuanbao.appKey and channels.yuanbao.appSecret",
			elapsed: Date.now() - startTime,
		};
	}
	...
	if (!testUserId) {
		return {
			...
			ok: false,
			error:
				"No Yuanbao recipient configured. Set channels.yuanbao.allowFrom or start one DM session first",
			elapsed: Date.now() - startTime,
		};
	}
	...
	try {
		const { getSignToken, YuanbaoWsClient, sendYuanbaoMessage } =
			await loadYuanbaoRuntime();
		const tokenData = await getSignToken(account);
		const botId =
			typeof tokenData?.bot_id === "string" && tokenData.bot_id.trim()
				...
				callbacks: {
					...
					onClose: (code: number, reason: string) =>
						finish(() =>
							reject(
								new Error(
									`Yuanbao WebSocket closed before ready: ${code} ${reason || ""}`.trim(),
								),
							),
						),
				},
...
async function getQqbotAccessToken(
	...
	try {
		...
		if (!resp.ok) {
			return {
				ok: false,
				error: `Token API HTTP ${resp.status}: ${(raw || "").slice(0, 180)}`,
			};
		}
		if (!data?.access_token) {
			return {
				ok: false,
				error: `Token API invalid response: ${(raw || "").slice(0, 180)}`,
			};
		}
...
async function testWhatsapp(
	...
	if (!testUserId) {
		return {
			...
			ok: false,
			error:
				"No WhatsApp recipient configured. Set channels.whatsapp.allowFrom or start one DM session first",
			elapsed: Date.now() - startTime,
		};
	}
...
async function testQqbot(
	...
	if (!creds) {
		return {
			...
			ok: false,
			error:
				"QQBot credentials missing. Configure channels.qqbot.appId/clientSecret (or accounts)",
			elapsed: Date.now() - startTime,
		};
	}

	const tokenResult = await getQqbotAccessToken(
		creds.appId,
		creds.clientSecret,
	);
...
export async function POST(request: Request) {
	...
	try {
		...
		const specialPlatformNames = new Set([
			"feishu",
			"discord",
			"telegram",
			"whatsapp",
		...
		for (const agent of agentList) {
			...
			if (
				account?.appId &&
				account.appSecret &&
				!testedFeishuAccounts.has(accountId)
			) {
			...
			if (id === "main" && discordConfig.enabled) {
				...
				const source: "session" | "allowFrom" | "none" = recentDmUser
					? "session"
					: allowFromUser
						? "allowFrom"
						: "none";
			...
			if (
				qqbotConfig &&
				qqbotConfig.enabled !== false &&
				(id === "main" || hasQqbotBinding)
			) {
				const qqbotBinding = bindings.find(
					(b: any) => b.agentId === id && b.match?.channel === "qqbot",
				);
				const qqbotAccountId =
					typeof qqbotBinding?.match?.accountId === "string" &&
				...
				const allowFromUser = normalizeQqbotTarget(
					getQqbotAllowlistUser(qqbotConfig, qqbotAccountId),
				);
			...
			for (const [channelName, channelConfig] of Object.entries(channels)) {
				...
				if (
					...
					(channelConfig as any).enabled === false
				)
					continue;
				...
				const yuanbaoDmContext =
					channelName === "yuanbao" ? getYuanbaoDmContext(id) : null;
				const recentDmUser =
					channelName === "yuanbao"
