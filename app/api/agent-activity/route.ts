...
function parseRecordTimestamp(record: unknown): number {
	...
	if (typeof rec.timestamp === "number" && Number.isFinite(rec.timestamp))
...
function inferCronOwnerAgentId(job: CronStoreJob): string {
	...
	if (
		...
		job.sessionKey.startsWith("agent:")
	) {
...
function deriveCronSummaryFromJob(job: CronStoreJob): string | undefined {
	const lastError =
		typeof job.state?.lastError === "string"
			? truncateSummary(job.state.lastError)
			: "";
...
function inferCronStatusFromTranscript(
	lines: string[],
	updatedAt: number,
): {
	isRunning: boolean;
	...
	for (const line of lines) {
		...
		if (record.type === "message") {
			...
			if (role === "assistant") {
				...
				const stopReason =
					typeof msg.stopReason === "string" ? msg.stopReason : "";
	...
	if (lastAssistantError || lastToolError) {
		return {
			...
			durationMs:
				firstAt > 0 && lastAt >= firstAt ? lastAt - firstAt : undefined,
		};
	}
...
function extractChildSessionKeyFromPayload(payload: unknown): string | null {
	...
	if (typeof direct === "string" && direct.includes(":subagent:"))
		return direct;

	const details = data.details;
	if (details && typeof details === "object") {
		const fromDetails = (details as Record<string, unknown>).childSessionKey;
		if (typeof fromDetails === "string" && fromDetails.includes(":subagent:"))
			return fromDetails;
	}
	return null;
}
...
function extractChildSessionKeyFromToolResultMessage(
	message: unknown,
): string | null {
...
function resolveSubagentSessionId(
	...
	if (typeof fromIndex === "string" && fromIndex.trim())
...
async function parseSubagentActivityEvents(
	...
	try {
		...
		for (let i = 0; i < lines.length; i++) {
			...
			if (role === "toolResult") {
				const toolName =
					typeof msg.toolName === "string" ? msg.toolName.trim() : "";
				const details =
				...
				for (let bi = 0; bi < blocks.length; bi++) {
					const block = blocks[bi];
					if (!block || typeof block !== "object") continue;
					const normalized = normalizeActivityText(
		...
		for (const event of events) {
			...
			if (typeof lastAt === "number" && Math.abs(event.at - lastAt) <= 1500)
				continue;
...
async function parseSubagentsFromSessionFile(
	...
	try {
		...
		const activeSubtasks = new Map<
			string,
			{ label: string; at: number; childSessionKey?: string }
		>();
				...
				if (record.type === "assistant" && record.message?.content) {
					...
					for (const block of blocks) {
						...
						if (typeof block.name === "string" && isSpawnTool(block.name)) {
							activeSubtasks.set(block.id, {
								label: pickSubagentLabel(block.input),
								at: eventAt,
							});
						...
						if (
							...
							isSubtaskDescription(block.input.description)
						) {
							activeSubtasks.set(block.id, {
					...
					if (role === "assistant") {
						for (const block of blocks) {
							if (
								...
								if (typeof block.name === "string" && isSpawnTool(block.name)) {
									activeSubtasks.set(block.id, {
										label: pickSubagentLabel(block.arguments),
										at: eventAt,
									});
									spawnToolIds.add(block.id);
								} else if (
									typeof block.arguments?.description === "string" &&
									isSubtaskDescription(block.arguments.description)
								) {
									activeSubtasks.set(block.id, {
										label: block.arguments.description,
										at: eventAt,
									});
								}
							} else if (
								...
								if (isSubtaskDescription(block.input.description))
									activeSubtasks.set(block.id, {
										label: block.input.description,
										at: eventAt,
									});
						...
						if (toolCallId && spawnToolIds.has(toolCallId)) {
							const childSessionKey =
								extractChildSessionKeyFromToolResultMessage(msg);
						...
						if (completedLabel) {
							for (const [id, state] of activeSubtasks.entries()) {
								if (
									state.label === completedLabel ||
									state.label.includes(completedLabel) ||
									completedLabel.includes(state.label)
								) {
		...
		for (const [toolId, state] of activeSubtasks.entries()) {
			...
			if (state.childSessionKey) {
				activityEvents = await parseSubagentActivityEvents(
					agentSessionsDir,
...
async function parseCronJobs(
	agentSessionsDir: string,
	cronJobsForAgent: CronStoreJob[],
): Promise<CronJobInfo[]> {
	if (cronJobsForAgent.length === 0) return [];

	const sessionsIndexPath = path.join(agentSessionsDir, "sessions.json");
	const sessionsIndex = existsSync(sessionsIndexPath)
		? (JSON.parse(await fs.readFile(sessionsIndexPath, "utf8")) as Record<
				string,
				any
			>)
		: {};
	...
	for (const job of cronJobsForAgent) {
		const entries = Object.entries(sessionsIndex)
			.filter(
				([sessionKey, meta]) =>
					sessionKey.includes(`:cron:${job.id}`) &&
					meta &&
					typeof (meta as any).sessionId === "string",
			)
			.map(([sessionKey, meta]) => ({
				sessionKey,
				...
				updatedAt:
					typeof (meta as any).updatedAt === "number"
						...
						: 0,
				label:
					typeof (meta as any).label === "string"
						? (meta as any).label
						: undefined,
			}))
		...
		let transcriptStatus: ReturnType<
			typeof inferCronStatusFromTranscript
		> | null = null;

		if (latest) {
			const transcriptPath = path.join(
				agentSessionsDir,
				`${latest.sessionId}.jsonl`,
			);
			if (existsSync(transcriptPath)) {
				const transcript = await fs.readFile(transcriptPath, "utf8");
				transcriptStatus = inferCronStatusFromTranscript(
		...
		cronJobs.push({
			key: latest?.sessionKey?.includes(":run:")
				? latest.sessionKey.split(":run:")[0]
				: latest?.sessionKey ||
					`agent:${inferCronOwnerAgentId(job)}:cron:${job.id}`,
			jobId: job.id,
			label: normalizeCronLabel(job.name || latest?.label, fallbackLabel),
			isRunning:
				transcriptStatus?.isRunning ||
				mapCronStatus(state.lastStatus) === "running",
			lastRunAt:
				typeof state.lastRunAtMs === "number"
					? state.lastRunAtMs
					: latest?.updatedAt || 0,
			nextRunAt:
				typeof state.nextRunAtMs === "number" ? state.nextRunAtMs : undefined,
			durationMs:
				typeof state.lastDurationMs === "number"
					? state.lastDurationMs
					: transcriptStatus?.durationMs,
			lastStatus:
				transcriptStatus?.lastStatus || mapCronStatus(state.lastStatus),
			lastSummary:
				transcriptStatus?.lastSummary || deriveCronSummaryFromJob(job),
			consecutiveFailures:
				...
				for (const agent of agentList) {
					...
					if (
						...
						cronJobs = await parseCronJobs(
							agentSessionsDir,
							liveCronJobs.filter(
								(job) => inferCronOwnerAgentId(job) === agent.id,
							),
						);
