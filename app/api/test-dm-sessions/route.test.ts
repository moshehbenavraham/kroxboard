...
describe("POST /api/test-dm-sessions", () => {
	let tempOpenclawHome = "";

	beforeEach(() => {
		vi.resetModules();
		tempOpenclawHome = fs.mkdtempSync(
			path.join(os.tmpdir(), "kroxboard-test-dm-sessions-"),
		);
	...
	it("returns 403 when outbound tests are disabled", async () => {
		...
		const response = await route.POST(
			new Request("http://localhost:3000/api/test-dm-sessions", {
				method: "POST",
	...
	it("rejects cross-origin DM diagnostics before calling the gateway", async () => {
	...
	it("returns DM diagnostic results", async () => {
		vi.stubGlobal(
			"fetch",
			vi.fn(
				async () =>
					new Response(
						JSON.stringify({
							choices: [{ message: { content: "OK from DM gateway" } }],
						}),
						{ status: 200 },
					),
			),
	...
	it("returns 429 after repeated DM diagnostic runs", async () => {
		vi.stubGlobal(
			"fetch",
			vi.fn(
				async () =>
		...
		await expect(denied.json()).resolves.toMatchObject({
			rateLimit: {
				capability: "dm_session_diagnostic_batch",
			},
		});
	});
});
