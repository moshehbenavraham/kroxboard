...
import {
	type AgentActivity,
	syncAgentsToOffice,
} from "@/lib/pixel-office/agentBridge";
...
import {
	type GatewaySreState,
	OfficeState,
} from "@/lib/pixel-office/engine/officeState";
import type {
	ContributionData,
	EditorRenderState,
} from "@/lib/pixel-office/engine/renderer";
...
import { EditTool, TileType } from "@/lib/pixel-office/types";
import {
	AgentCard,
	type AgentCardAgent,
...
function _MiniSparkline({
	...
	const pts = data.map((v, i) => {
		...
		const y =
			v === 0
				? height - pad
				: height - pad - ((v - min) / range) * (height - pad * 2 - 2);
		return { x, y, v };
	});
...
function _getLayoutContentBounds(layout: OfficeLayout): {
	minCol: number;
	maxCol: number;
	minRow: number;
	maxRow: number;
...
export default function PixelOfficePage() {
	...
	const nextIdRef = useRef<{ current: number }>({
		current: cachedNextCharacterId,
	});
	...
	const prevAgentStatesRef = useRef<Map<string, string>>(
		new Map(cachedPrevAgentStates),
	);
	...
	const [_editorTick, setEditorTick] = useState(0);
	...
	const [broadcasts, setBroadcasts] = useState<
		Array<{ id: number; emoji: string; text: string }>
	>([]);
	const [showActivityHeatmap, setShowActivityHeatmap] = useState(false);
	const activityHeatmapRef = useRef<Array<{
		agentId: string;
		grid: number[][];
	}> | null>(null);
	...
	> | null>(null);
	const [cachedPlatformTestResults, setCachedPlatformTestResults] =
		useState<Record<string, PlatformTestResult | null> | null>(null);
	const [cachedSessionTestResults, setCachedSessionTestResults] =
		useState<Record<string, AgentSessionTestResult | null> | null>(null);
	...
	const [_floatingTick, setFloatingTick] = useState(0);
	...
	const refreshGatewayHealthSnapshot = useCallback(async () => {
		...
		try {
			...
			if (
				...
				responseMs > GATEWAY_DEGRADED_LATENCY_MS
			) {
				rawStatus = "degraded";
			} else {
				rawStatus = "healthy";
		...
		if (rawStatus === "down") {
			...
			if (prev === "down" || prev === "degraded") {
				if (
					gatewayHealthyStreakRef.current >=
					GATEWAY_SRE_RECOVERY_SUCCESS_THRESHOLD
				) {
					effective = "healthy";
				}
			} else {
	...
	// Game loop
	useEffect(() => {
		if (!canvasRef.current || !officeRef.current || !containerRef.current)
			return;
		...
		const render = (time: number) => {
			...
			if (isMobileViewport) {
				...
				const fitW =
					Math.max(1, width - MOBILE_FIT_PADDING_PX * 2) / Math.max(1, baseW);
				const fitH =
				...
				const basePanY =
					minPanY > maxPanY ? minPanY : Math.min(maxPanY, Math.max(minPanY, 0));
				...
				const centerOffsetY = (height - mapH) / 2;
				const topExtraPx =
					DESKTOP_TOP_EXTRA_TILES * TILE_SIZE * zoomRef.current;
				const minPanY =
					topExtraPx + DESKTOP_TOP_SAFE_PADDING_PX - centerOffsetY;
			...
			if (ctx) {
				...
				if (editor.isEditMode) {
					...
					const selItem = sel
						? office.layout.furniture.find((f) => f.uid === sel)
						: null;
				...
				for (const ch of office.getCharacters()) {
					...
					const anchorX = ox + ch.x * zoom;
					const anchorY =
						containerTop + oy + (ch.y - (isSreBlackword ? 24 : 10)) * zoom;
	...
	useEffect(() => {
		fetch("/api/pixel-office/idle-rank")
			.then((r) => (r.ok ? r.json() : null))
			.then((data) => {
				if (data?.agents) idleRankRef.current = data.agents;
			})
			.catch(() => {});
	}, []);
			...
			if (editor.activeTool === EditTool.FURNITURE_PLACE) {
				...
				if (
					...
					if (editor.activeTool === EditTool.TILE_PAINT) {
						...
						const currentTile =
							office.layout.tiles[row * office.layout.cols + col];
						...
						applyEdit(
							paintTile(office.layout, col, row, TileType.VOID as TileTypeVal),
						);
					}
				}
			} else {
	...
	const handleMouseDown = (e: React.MouseEvent) => {
		...
		if (!editor.isEditMode) {
			...
			if (e.button === 0) {
				...
				const clickedServer = office.layout.furniture.some((f) => {
					if (f.uid !== "server-b-left" && f.type !== "server_rack")
						return false;
					const entry = getCatalogEntry(f.type);
					if (!entry) return false;
					return (
				...
				if (clickedCamera) {
					...
					if (charId !== null) {
						...
						if (!found) {
							...
							if (
								clickedCh?.systemRoleType === "gateway_sre" &&
								isMobileViewport
							) {
								setServerTooltip({ open: true, x: clickX, y: clickY });
								void refreshGatewayHealthSnapshot();
							} else if (
								clickedCh?.isSubagent &&
								clickedCh.parentAgentId != null
							) {
		...
		if (e.button === 0) {
			...
			if (
				...
				const dir = getGhostBorderDirection(
					col,
					row,
					office.layout.cols,
					office.layout.rows,
				...
				if (
					...
					editor.activeTool =
						editor.activeTool === EditTool.EYEDROPPER
							? EditTool.TILE_PAINT
							: editor.activeTool;
					forceEditorUpdate();
				}
				...
				if (pickedItem) {
					...
					editor.pickedFurnitureColor = pickedItem.color
						? { ...pickedItem.color }
						: null;
					editor.activeTool = EditTool.FURNITURE_PLACE;
					forceEditorUpdate();
	...
	useEffect(() => {
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			if (e.key === "r" || e.key === "R") {
				...
				if (editor.selectedFurnitureUid) {
					applyEdit(
						toggleFurnitureState(office.layout, editor.selectedFurnitureUid),
					);
				}
	...
	const resetView = useCallback(() => {
		zoomRef.current = isMobileViewport
			? MOBILE_CANVAS_ZOOM
			: DESKTOP_CANVAS_ZOOM;
	...
	const getHoveredAgentInfo = useCallback(() => {
		...
		if (
			hoveredCharacter?.isSubagent &&
			hoveredCharacter.parentAgentId != null
		) {
			let parentAgentId: string | null = null;
			for (const [aid, cid] of map.entries()) {
	...
	const selectedItem = editor.selectedFurnitureUid
		? officeRef.current?.layout.furniture.find(
				(f) => f.uid === editor.selectedFurnitureUid,
			)
		: null;
	...
	const renderAgentChip = (agent: AgentActivity, mobileGrid = false) => {
		...
		const parentAgentIdFromKey = isTempWorker
			? agent.agentId.split(":")[1] || ""
			: "";
		const tempWorkerOwner = isTempWorker
			? agent.name.replace(
					new RegExp(`^${t("pixelOffice.tempWorker")}\\s*`),
					"",
			...
			<div key={agent.agentId} className="group relative overflow-visible">
				<div
					className={`pixel-agent-chip inline-flex h-8 items-center overflow-hidden rounded-lg border transition-colors ${
						mobileGrid
							? "w-full min-w-0 gap-1.5 px-2 py-1.5"
					...
					{...(agent.state === "working"
						...
						: {})}
				>
					<span className={mobileGrid ? "shrink-0 text-sm" : ""}>
						{agent.emoji}
					</span>
					{isTempWorker ? (
						<span
						...
						>
							<span
								className={`${mobileGrid ? "text-[10px]" : "text-[12px]"} truncate`}
							>
						...
						<span
							className={mobileGrid ? "min-w-0 text-xs truncate" : "text-sm"}
						>
							{agent.name}
						</span>
					)}
	...
	return (
			...
			{floatingCodeRef.current.map((fc) => (
				...
				>
					<span
						...
						style={{
							backgroundColor:
								fc.kind === "sre" ? "rgba(28,6,6,0.9)" : "rgba(0,0,0,0.72)",
							color: fc.kind === "sre" ? "#DC2626" : "#4ade80",
							border:
								fc.kind === "sre"
									? "1px solid rgba(127,29,29,0.85)"
			...
			<div className="flex flex-col gap-2 p-3 md:p-4 border-b border-[var(--border)]">
				<div className="flex items-center justify-between gap-2">
					...
					<div className="flex gap-2">
						...
						>
							{isEditMode
								? t("pixelOffice.exitEdit")
								: t("pixelOffice.editMode")}
						</button>
					</div>
				</div>
					...
					<div className="absolute top-3 left-1/2 -translate-x-1/2 z-10 flex flex-col gap-2 pointer-events-none">
						{broadcasts.map((b) => (
							<div
								...
								style={{
									animation:
										"broadcastIn 0.3s ease-out, broadcastOut 0.5s ease-in 4.5s forwards",
								}}
							>
								{b.text}
							</div>
        ...
        `}</style>
									...
									<div className="space-y-0.5 text-[var(--text-muted)]">
										<div className="flex justify-between gap-4">
											...
											<span className="text-[var(--text)]">
												{t(
													`pixelOffice.gatewaySre.status.${hoveredInfo.gatewaySre.status}`,
												)}
											</span>
										</div>
										<div className="flex justify-between gap-4">
									...
									<div className="flex items-center gap-1.5 mb-1.5">
										...
										<span className="font-semibold text-[var(--text)]">
											{hoveredInfo.isSubagent
												? t("pixelOffice.tempWorker")
												: hoveredInfo.agent?.name}
										</span>
									</div>
									{hoveredInfo.isSubagent ? (
										<div className="text-[var(--text-muted)]">
											{hoveredInfo.parentAgentId || "unknown"}{" "}
											{t("pixelOffice.tempWorker.createdBy")}
										</div>
									) : (
										<div className="space-y-0.5 text-[var(--text-muted)]">
											<div className="flex justify-between gap-4">
											...
											<div className="flex justify-between gap-4">
												...
												<span className="text-[var(--text)]">
													{hoveredInfo.stats
														? formatTokens(hoveredInfo.stats.totalTokens)
														: "--"}
												</span>
											</div>
											<div className="flex justify-between gap-4">
								...
								<div className="space-y-0.5 text-[var(--text-muted)]">
									<div className="flex justify-between gap-4">
										...
										<span className={statusColor}>
											{t(`pixelOffice.serverStatus.${status}`)}
										</span>
									</div>
									<div className="flex justify-between gap-4">
										<span>{t("pixelOffice.gatewaySre.responseMs")}</span>
										<span className="text-[var(--text)]">
											{snapshot.responseMs != null
												? `${snapshot.responseMs}ms`
												: "--"}
										</span>
									</div>
									{snapshot.error && (
				...
				{selectedAgentId &&
					!isEditMode &&
					(() => {
						const runtimeAgent = agents.find(
							...
							>
								...
								>
									...
									<AgentCard
										...
										sessionTestResult={
											cachedSessionTestResults?.[selectedAgentId]
										}
					...
					(() => {
						...
						return (
							...
							>
								...
								<div className="space-y-0.5 text-[var(--text-muted)]">
									<div>
										{subagentCreatorInfo.parentAgentId}{" "}
										{t("pixelOffice.tempWorker.createdBy")}
									</div>
								</div>
							</div>
				...
				{showModelPanel && !isEditMode && (
					<div
						...
						onClick={() => {
							if (
								isMobileViewport &&
								performance.now() - modelPanelOpenedAtRef.current < 280
							)
								return;
							setShowModelPanel(false);
						}}
					>
						<div
							className={modalPanelClass("w-80")}
							onClick={(e) => e.stopPropagation()}
						>
				...
				{showTokenRank &&
					...
					(() => {
						...
						const handleCloseTokenRankOverlay = () => {
							...
							if (
								isMobileViewport &&
								performance.now() - tokenRankOpenedAtRef.current < 280
							)
								return;
							setShowTokenRank(false);
						};
						return (
							<div
								className={modalOverlayClass}
								onClick={handleCloseTokenRankOverlay}
							>
								<div
												...
												return (
													<div key={a.agentId}>
														...
														<div className="flex gap-3 text-[10px] text-[var(--text-muted)] mt-0.5">
															<span>
																{t("pixelOffice.idleRank.online")}{" "}
																{a.onlineMinutes}m
															</span>
															<span>
																{t("pixelOffice.idleRank.active")}{" "}
																{a.activeMinutes}m
															</span>
															<span>
																{t("pixelOffice.idleRank.idle")} {a.idleMinutes}
																m
															</span>
														</div>
													</div>
				...
				{fullscreenPhoto && photographRef.current && (
					<div
						...
						onClick={() => {
							if (
								isMobileViewport &&
								performance.now() - fullscreenPhotoOpenedAtRef.current < 280
							)
								return;
							setFullscreenPhoto(false);
						}}
