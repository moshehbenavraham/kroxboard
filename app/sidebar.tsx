...
function navPalette(active: boolean): PixelPalette {
	return active
		? {
				base: "var(--accent)",
				shade: "color-mix(in srgb, var(--accent) 72%, black)",
				light: "color-mix(in srgb, var(--accent) 55%, white)",
			}
		: {
				base: "var(--text)",
				shade: "color-mix(in srgb, var(--text) 62%, black)",
				light: "color-mix(in srgb, var(--text) 35%, white)",
			};
}

function NavPixelIcon({
	name,
	active,
}: {
	name: NavIconName;
...
const NAV_ITEMS: {
	group: string;
	items: { href: string; icon: NavIconName; labelKey: string }[];
}[] = [
	{
		group: "nav.overview",
		items: [
	...
	{
		group: "nav.config",
		items: [{ href: "/skills", icon: "skills", labelKey: "nav.skills" }],
	},
];
...
export function Sidebar() {
	...
	const [mobileOpenclawVersion, setMobileOpenclawVersion] = useState<
		string | null
	>(null);
	const [experimentOpen, setExperimentOpen] = useState(false);
	const [bugsEnabled, setBugsEnabled] = useState(false);
	const [bugsCount, setBugsCount] = useState(5);
	const [logoCarry, setLogoCarry] = useState<{
		active: boolean;
		dx: number;
		dy: number;
		angle: number;
	...
	const [manualLogoOffset, setManualLogoOffset] = useState<{
		dx: number;
		dy: number;
	}>({ dx: 0, dy: 0 });
	...
	useEffect(() => {
		const onStart = () =>
			setLogoCarry((s) => ({ ...s, active: true, hidden: false }));
		const onStop = () =>
			setLogoCarry({ active: false, dx: 0, dy: 0, angle: 0, hidden: false });
	...
	useEffect(() => {
		const syncFromStorage = () => {
			...
			const count = Math.max(
				0,
				Math.min(BUGS_MAX, Number.isFinite(raw) ? raw : 5),
			);
	...
	useEffect(() => {
		...
		const onMouseMove = (e: MouseEvent) => {
			...
			const nextDx =
				dragRef.current.originDx + (e.clientX - dragRef.current.startX);
			const nextDy =
				dragRef.current.originDy + (e.clientY - dragRef.current.startY);
			if (
				Math.abs(nextDx - dragRef.current.originDx) > 3 ||
				Math.abs(nextDy - dragRef.current.originDy) > 3
			) {
			...
			if (Math.abs(moveX) + Math.abs(moveY) > 0.2) {
				const targetAngle = Math.max(
					-0.95,
					Math.min(0.95, Math.atan2(moveY, moveX) * 0.65),
				);
		...
		<>
											...
											<div className="mt-2 space-y-2 rounded-lg border border-[var(--border)] bg-[var(--card)] p-2">
												...
												<label className="flex items-center justify-between gap-2 px-2 py-1.5 text-xs rounded-lg border bg-[var(--card)] border-[var(--border)] text-[var(--text-muted)]">
													...
													<input
														...
														value={bugsCount}
														onChange={(e) =>
															onBugCountChange(Number(e.target.value))
														}
			...
			>
				...
				<div
					...
					style={{ padding: collapsed ? "16px 0" : "16px 20px" }}
				>
					{collapsed ? (
