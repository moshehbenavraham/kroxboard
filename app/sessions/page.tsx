						...
						{agent.session && (
							<div className="space-y-1 text-xs text-[var(--text-muted)]">
								...
								<div className="flex justify-between">
									...
									<span className="text-[var(--text)]">
										{(agent.session.totalTokens / 1000).toFixed(1)}k
									</span>
								</div>
...
function SessionList({ agentId }: { agentId: string }) {
	...
	const [testResults, setTestResults] = useState<
		Record<
			string,
			{ status: string; elapsed?: number; reply?: string; error?: string }
		>
	>({});
	...
	function getTypeLabel(type: string): {
		...
		const label = t(
			TYPE_EMOJI_COLOR[type] ? labelKey : "sessions.type.unknown",
		);
		return { ...info, label };
	}
						...
						>
							<div className="flex flex-col gap-2 mb-2 md:flex-row md:items-center md:justify-between">
								...
								<div className="flex items-center gap-2 md:self-auto self-start">
									...
									>
										{testResults[s.key]?.status === "testing"
											? t("sessions.testing")
											: t("sessions.test")}
									</button>
									<span className="text-xs text-[var(--text-muted)]">
								...
								testResults[s.key].status !== "testing" && (
									...
									>
										...
										{testResults[s.key].elapsed && (
											<span className="ml-2">
												{t("sessions.testTime")}:{" "}
												{(testResults[s.key].elapsed! / 1000).toFixed(1)}s
											</span>
										)}
										{testResults[s.key].reply && (
											<span className="ml-2 opacity-80 break-all">
												{t("sessions.testReply")}: {testResults[s.key].reply}
											</span>
										)}
								...
								<div className="mb-2">
									<div className="flex items-center justify-between text-xs text-[var(--text-muted)] mb-1">
										...
										<span>
											...
											{(s.contextTokens / 1000).toFixed(0)}k (
							...
							<div className="flex items-center justify-between gap-2 text-xs text-[var(--text-muted)]">
								<span className="font-mono text-[10px] opacity-60 truncate max-w-[56vw] md:max-w-none">
									{s.key}
								</span>
