...
function PlatformBadge({
	...
	const knownMeta: Record<
		string,
		{
			remoteLogoSrc: string;
	...
	> = {
		...
		wecom: {
			...
			badgeStyle:
				"bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 hover:bg-emerald-500/40 hover:border-emerald-400",
		},
	};
...
function AgentStatusBadge({ state, t }: { state?: string; t: TFunc }) {
	const config: Record<
		string,
		{ dot: string; text: string; color: string; pulse?: boolean }
	> = {
		working: {
			dot: "bg-green-400",
			text: t("agent.status.working"),
			color: "text-green-400",
			pulse: true,
		},
		online: {
			dot: "bg-green-400",
			text: t("agent.status.online"),
			color: "text-green-400",
		},
		idle: {
			dot: "bg-yellow-400",
			text: t("agent.status.idle"),
			color: "text-yellow-400",
		},
		offline: {
			dot: "bg-red-400",
			text: t("agent.status.offline"),
			color: "text-red-400",
		},
	};
	...
	return (
		<span className={`inline-flex items-center gap-1 text-xs ${c.color}`}>
			<span
				className={`w-2.5 h-2.5 rounded-full ${c.dot} ${c.pulse ? "animate-pulse" : ""}`}
			/>
			{c.text}
		</span>
	);
...
export function AgentCard({
	...
	const knownModelRefs = new Set(
		(modelOptions || []).flatMap((group) =>
			group.models.map((model) => `${group.providerId}/${model.id}`),
		),
	);
						...
						<div className="mt-2 rounded-lg border border-[var(--border)] bg-[var(--bg)] p-2 space-y-2">
							...
							>
								{!currentModelKnown && (
									<option
										value={agent.model}
									>{`${t("agent.currentUnknownModel")}: ${agent.model}`}</option>
								)}
							...
							<div className="flex items-center gap-2">
								<button
									...
									disabled={
										isSavingModel || !draftModel || draftModel === agent.model
									}
				...
				<div>
					...
					<div className="flex flex-col gap-1">
						{agent.platforms.map((p, i) => {
							...
							const pResult = platformTestResults
								? platformTestResults[pKey]
								: undefined;
							const dmKey = `${agent.id}:${p.name}`;
							const dmResult = dmSessionResults
								? dmSessionResults[dmKey]
								: undefined;
							return (
				...
				{agent.session && (
					<div className="pt-1 mt-1 border-t border-[var(--border)]">
						...
						<div className="flex items-center justify-between text-xs mt-1">
							...
							<span
								className="text-[var(--text)] cursor-help"
								title={t("agent.totalTokenTip")}
							>
						...
						{agent.session.lastActive && (
							<div className="flex items-center justify-between text-xs mt-1">
								...
								<span className="text-[var(--text)]">
									{formatTimeAgo(agent.session.lastActive)}
								</span>
							</div>
						)}
						<div className="flex items-center justify-between text-xs mt-1">
							...
							{agent.session.weeklyResponseMs && (
								<MiniSparkline data={agent.session.weeklyResponseMs} />
							)}
							{(() => {
								...
								return (
									<span
										title={t("agent.todayAvgResponseTip")}
										className={`font-mono cursor-help ${colorClass}`}
									>
										{val ? formatMs(val) : "--"}
