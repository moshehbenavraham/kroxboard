			...
			<div className="flex flex-col gap-3 mb-6 md:mb-8 md:flex-row md:items-center md:justify-between">
				<div>
					...
					<p className="text-[var(--text-muted)] text-sm mt-1">
						{t("models.totalPrefix")} {data.providers.length}{" "}
				...
				{data.providers.map((provider) => (
					...
					>
								...
								<div className="flex items-center gap-1">
									...
									{provider.usedBy.map((a) => (
										<span
											key={a.id}
											title={a.id}
											className="px-2 py-0.5 rounded-full bg-[var(--bg)] text-xs font-medium"
										>
											{a.emoji} {a.name || a.id}
										</span>
									))}
						...
						{provider.models.length > 0 ? (
							<div>
								{(() => {
									const hasDetail = provider.models.some(
										(m: any) => m.contextWindow || m.maxTokens,
									);
									return (
										<>
														...
														>
															<div className="flex items-start justify-between gap-2">
																<div className="min-w-0">
																	<div className="font-mono text-xs text-[var(--accent)] truncate">
																		{m.id}
																	</div>
															...
															<div className="mt-2 grid grid-cols-2 gap-2 text-xs">
																...
																<div className="rounded border border-[var(--border)] bg-[var(--card)] px-2 py-1">
																	<div className="text-[var(--text-muted)]">
																		{t("models.colAvgResponse")}
																	</div>
																	<div className="text-amber-400 font-mono">
																		{stat ? formatMs(stat.avgResponseMs) : "-"}
																	</div>
																</div>
																{hasDetail && (
																	<div className="rounded border border-[var(--border)] bg-[var(--card)] px-2 py-1">
															...
															<div className="mt-2 flex items-center justify-between gap-2">
																...
																{result && (
																	<span
																		className={`text-[10px] ${result.ok ? "text-green-400" : "text-red-400"} truncate max-w-[56vw]`}
																		title={
												...
												<table className="w-full text-sm">
													<thead>
														<tr className="text-[var(--text-muted)] text-xs border-b border-[var(--border)]">
															...
															{hasDetail && (
																<th className="text-left py-2 pr-4">
																	{t("models.colMaxOutput")}
																</th>
															)}
															{hasDetail && (
																<th className="text-left py-2 pr-4">
																	{t("models.colInputType")}
																</th>
															)}
															{hasDetail && (
																<th className="text-left py-2 pr-4">
														...
														{provider.models.map((m) => {
															...
															return (
																...
																>
																	...
																	<td className="py-2 pr-4 text-right text-emerald-400 font-mono text-xs">
																		{stat
																			? formatTokens(stat.outputTokens)
																			: "-"}
																	</td>
																	<td className="py-2 pr-4 text-right text-amber-400 font-mono text-xs">
																		{stat ? formatMs(stat.avgResponseMs) : "-"}
																	</td>
																	<td className="py-2 text-center">
																		<div className="flex flex-col items-center gap-1">
																			...
																			{result && (
																				<span
																					className={`text-[10px] max-w-[140px] truncate ${result.ok ? "text-green-400" : "text-red-400"}`}
																					title={
																						result.ok
																							? result.text
																							: result.error
							...
							<div>
								...
								{(() => {
									const providerStats = Object.values(modelStats).filter(
										(s) => s.provider === provider.id,
									);
									if (providerStats.length === 0) return null;
									const _totalInput = providerStats.reduce(
										(s, m) => s + m.inputTokens,
										0,
									);
									const _totalOutput = providerStats.reduce(
										(s, m) => s + m.outputTokens,
										0,
									);
									const allRt = providerStats.filter(
										(m) => m.avgResponseMs > 0,
									);
									const _avgRt =
										allRt.length > 0
											? Math.round(
													allRt.reduce((s, m) => s + m.avgResponseMs, 0) /
														allRt.length,
												)
											: 0;
									return (
