...
function stripOpaqueSheetBackground(sprite: SpriteData): SpriteData {
	...
	const corners = [
		...
		sprite[0][width - 1],
		sprite[height - 1][0],
		sprite[height - 1][width - 1],
	];
	const threshold = Math.max(
		140,
		Math.min(...corners.map((pixel) => brightness(pixel))) - 12,
	);
...
export async function loadWallPNG(): Promise<boolean> {
	try {
		...
		for (let i = 0; i < 16; i++) {
			...
			sprites.push(
				extractRegion(sheet, col * PIECE_W, row * PIECE_H, PIECE_W, PIECE_H),
			);
		}
